...
import {
  ...
  type AgendaFirmasWeeklyLocation,
  type HhmmTime,
...
type EditableLocation = AgendaFirmasWeeklyLocation & { key: string };

function toEditableLocations(
  locations: AgendaFirmasWeeklyConfig["locations"],
): EditableLocation[] {
...
export function AgendaFirmasWeeklySupabaseSection({ canEdit }: Props) {
  ...
  async function save() {
    ...
    const normalizedLocations: AgendaFirmasWeeklyLocation[] = [];
    ...
    try {
      ...
      setSaveOk(
        result.created
          ? "Configuración de firmas creada en Supabase."
          : "Configuración de firmas actualizada en Supabase.",
      );
    } catch (err) {
      setSaveError(
        err instanceof AgendaFirmasSupabaseError
  ...
  if (loading) {
    return (
      <div className="rounded-xl border border-slate-200/90 bg-white p-3 text-xs text-slate-600 sm:p-4">
        Cargando configuración de firmas (Supabase)…
      </div>
    );
  }
  ...
  if (!canEdit) {
    return (
      <section className="rounded-xl border border-slate-200/90 bg-white p-3 shadow-sm sm:p-4">
        <h2 className="text-sm font-semibold text-slate-900">Agenda firmas (Supabase)</h2>
  ...
  return (
    <section className="rounded-xl border border-violet-200 bg-violet-50/40 p-3 shadow-sm sm:p-4">
      <div>
        <h2 className="text-sm font-semibold text-slate-900">Configuración de agendas</h2>
        <p className="mt-1 text-[11px] font-semibold text-slate-700">Bloque B: Firmas (Supabase)</p>
        <p className="mt-1 text-[11px] text-slate-600">
          Modelo semanal canónico vía RPC `upsert_agenda_config_firmas`. Sin calendario por día.
        </p>
        {meta ? (
      ...
      <div className="mt-3 space-y-3">
        ...
        <div className="rounded-lg border border-violet-100 bg-white p-3">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {AGENDA_FIRMAS_WEEKDAY_OPTIONS.map((day) => (
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <Button
          ...
          onClick={() => void save()}
        >
          {saving ? "Guardando…" : "Guardar firmas (Supabase)"}
        </Button>
