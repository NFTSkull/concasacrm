-- ConCasa CRM — P3M.1A RPC upsert_agenda_config_biometricos (config disponibilidad biométricos)

-- =============================================================================
-- agenda_biometricos_validate_config — validación estricta pre-persistencia
-- =============================================================================
...
COMMENT ON FUNCTION public.agenda_biometricos_validate_config(JSONB) IS
  'Valida estructura canónica de agenda_config biométricos antes de upsert (P3M.1).';

-- =============================================================================
-- agenda_biometricos_config_upsert_warnings — bookings futuros vs nueva config
-- =============================================================================
CREATE OR REPLACE FUNCTION public.agenda_biometricos_config_upsert_warnings(
  p_org_id UUID,
  p_config JSONB
)
RETURNS JSONB
...
BEGIN
  v_config := public.agenda_biometricos_normalize_config(p_config);
  ...
  IF COALESCE((v_config->>'enabled')::BOOLEAN, true) IS NOT TRUE THEN
    IF EXISTS (
      ...
      v_warnings := v_warnings || jsonb_build_array(
        'agenda biométricos deshabilitada con citas futuras activas'
      );
    END IF;
    RETURN v_warnings;
  END IF;
...
COMMENT ON FUNCTION public.agenda_biometricos_config_upsert_warnings(UUID, JSONB) IS
  'Advertencias no bloqueantes si un upsert reduce disponibilidad con bookings futuros activos.';

-- =============================================================================
-- upsert_agenda_config_biometricos
-- =============================================================================
...
COMMENT ON FUNCTION public.upsert_agenda_config_biometricos(JSONB, UUID) IS
  'Mesa admin / super_admin: upsert agenda_config biométricos (modelo semanal canónico).';

REVOKE ALL ON FUNCTION public.agenda_biometricos_validate_config(JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.agenda_biometricos_config_upsert_warnings(UUID, JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.upsert_agenda_config_biometricos(JSONB, UUID) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.upsert_agenda_config_biometricos(JSONB, UUID) FROM anon;

GRANT EXECUTE ON FUNCTION public.agenda_biometricos_validate_config(JSONB) TO authenticated;
GRANT EXECUTE ON FUNCTION public.agenda_biometricos_config_upsert_warnings(UUID, JSONB) TO authenticated;
GRANT EXECUTE ON FUNCTION public.upsert_agenda_config_biometricos(JSONB, UUID) TO authenticated;
