-- ConCasa CRM — P2C-16 hook retención en update_documento_revision
...
DECLARE
  ...
  v_retencion_correccion BOOLEAN := false;
BEGIN
  ...
  IF p_estatus = 'rechazado' AND v_doc.tipo_documento LIKE 'retencion_%' THEN
    UPDATE public.retencion_envios
    SET
      estado = 'correccion_requerida',
      updated_at = NOW()
    WHERE expediente_id = v_doc.expediente_id;

    IF FOUND THEN
      v_retencion_correccion := true;
    END IF;
  END IF;
  ...
  PERFORM public.log_action(
    ...
    jsonb_build_object(
      ...
      'comentario_mesa', v_comentario_final,
      'retencion_correccion_requerida', v_retencion_correccion
    )
  );
...
COMMENT ON FUNCTION public.update_documento_revision(UUID, public.estatus_revision, TEXT) IS
  'Mesa valida/rechaza documento. Rechazo retencion_* marca retencion_envios.correccion_requerida si existe envío.';
