-- ConCasa CRM — P2C-20 extender avanzar_etapa_operativa (transición 9→10)

-- =============================================================================
-- avanzar_etapa_operativa (transiciones 1→2 … 8→9 y 9→10)
-- =============================================================================
...
BEGIN
  ...
  ELSIF v_exp.etapa_actual = 9 THEN
    IF v_actor_role NOT IN ('mesa_admin', 'super_admin') THEN
      RAISE EXCEPTION 'avanzar_etapa_operativa: rol no autorizado para avance 9→10 (%)', v_actor_role
        USING ERRCODE = '42501';
    END IF;
...
COMMENT ON FUNCTION public.avanzar_etapa_operativa(UUID, TEXT) IS
  'Mesa avanza expediente 1→2 … 8→9 o 9→10 (firmas). Otras transiciones no permitidas.';
